...
#Run scripts
source("Variables_HRS.R", local = knitr::knit_global())
source("Variables_CAM.R", local = knitr::knit_global())
...
8. Adjust for inflation, add (H)OOPMDy (yearly data), R/S total income, WidowWaveR and WidowYearR
...
2. Calculate catastrophic health expenditure
    1. Ratio of out-of-pocket medical expenditure and income (household/respondent)
    2. Ratio of out-of-pocket medical expenditure and total consumption (household)
...
#Distribution of health expenditure before and after the loss of a spouse
hrs_widowL %>% 
  filter((WidowYearR >= -16) & (WidowYearR <= 16)) %>% 
  ggplot(aes(as.factor(WidowYearR), log(ROOPMDy))) +
    ...
    labs(title = "Distribution of Health Expenditure",
         ...
         y="Yearly OOP medical expenditure, log, inflation-adjusted")

#Health expenditure before and after the loss of a spouse
hrs_widowL %>% 
  filter((WidowYearR >= -16) & (WidowYearR <= 16)) %>% 
  group_by(WidowYearR) %>% 
  summarise(ROOPMDa = mean(ROOPMDy, na.rm=TRUE)) %>% 
  drop_na(ROOPMDa) %>% 
  
  ggplot(aes(WidowYearR, ROOPMDa)) + 
    geom_line() + 
    geom_point() +
    ...
    labs(title = "Health Expenditure Before and After the Loss of a Spouse",
         x="Years before and after loss of a spouse",
         y="Yearly OOP medical expenditure, inflation adjusted")
...
hrs_widowL %>% 
  ...
  ggplot(aes(WidowYearR, ROOPMDa)) + 
    ...
    scale_y_continuous(limits = c(0, NA),
                       expand = expansion(mult = c(0, 0.1))) + 
    geom_vline(xintercept = 0, color="grey") +
    labs(title = "Health Expenditure Before and After the Loss of a Spouse, by gender",
         x="Years before and after loss of a spouse",
         y="Yearly OOP medical expenditure, inflation adjusted") +
    facet_wrap(~RAGENDER)
```
